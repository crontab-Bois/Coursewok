...
3* )	echo "Option 3 selected"
	echo "Please enter the name of the script you would like to edit, this will be the same as the name of the script. For example ./job-scripts/test1.sh you would type test1.sh"
        read -r Edit

	#this line removes the job that the user has chosen to be edited.
	crontab -l | grep -v $Edit | crontab -

	echo "Please enter the new details of when you would like your job to run"
	echo "Choose a day of the week (0-7) (Sunday = 0 or 7)"
	echo "Enter an '*' if you would like all options selected:"
	while :; do
		read -r DofW
	...
	#when a cron job is created while no other jobs are active an error message will
   	#be returned '2>/dev/null' removes the message to avoid confusion.
	crontab -l > tempcron 2>/dev/null
	echo "$Minute $Hour $DofM $Month $DofW $Location/job-scripts/$Edit" >> tempcron
	crontab tempcron
	rm tempcron
	printf "\n";;
